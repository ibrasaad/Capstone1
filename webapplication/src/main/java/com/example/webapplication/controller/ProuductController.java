...
@DeleteMapping("/delete/{id}")
    ...
    @GetMapping("/range/{maxPrice}")
    public ResponseEntity<?> getProductsInRange(@PathVariable double maxPrice) {

        ArrayList<Product> products = productService.getProductsinRange(maxPrice);

        if (products.isEmpty()) {
            return ResponseEntity.status(400).body("no products in this range");
        }

        return ResponseEntity.status(200).body(products);
    }
    @PutMapping("/discount/{merchantId}/{productId}")
    public ResponseEntity<?> discountProduct(@PathVariable String merchantId,
                                             @PathVariable String productId) {

        boolean discount = merchantStockService.applyDiscountForProduct(merchantId, productId);

        if (!discount) {
            return ResponseEntity.status(400).body("merchant or product not found");
        }

        return ResponseEntity.status(200).body("discount applied");
    }
